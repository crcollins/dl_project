...
y = numpy.loadtxt("data/benzene_forces_ht.txt")[:NUM, :]
...
out = merge([Ept1, Ept2], mode=lambda x: (x[0] - x[1]) / EPS, output_shape=(1, ))
...
model.load_weights("/tmp/lstm_force_weights.hdf5")
model.compile(optimizer='adam', loss='mae')
print(model.summary())
pprint.pprint(model.get_config())

checkpointer = ModelCheckpoint(filepath="/tmp/lstm_force_weights.hdf5", verbose=1)
...
'''
model.save("models/lstm_force.keras")
