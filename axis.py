...
from keras.layers import Input, Dense, merge, Reshape, Lambda
...
axis_input = Input(shape=(2*100, ))
x2 = Dense(50, activation='relu')(axis_input)
aout = Dense(1)(x2)

axis_model = Model(axis_input, aout)


atom_input = Input(shape=(X.shape[2], ))
new_shape = Reshape([2*100, 3])(atom_input)

selectx = Lambda(lambda x: x[:, :, 0])(new_shape)
selecty = Lambda(lambda x: x[:, :, 1])(new_shape)
selectz = Lambda(lambda x: x[:, :, 2])(new_shape)

xaxis = axis_model(selectx)
yaxis = axis_model(selecty)
zaxis = axis_model(selectz)

axes = merge([xaxis, yaxis, zaxis], mode='concat')
